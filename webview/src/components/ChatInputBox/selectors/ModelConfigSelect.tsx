...
import {
  AVAILABLE_MODELS,
  type CodexFastMode,
  type CodexContextWindowPreset,
  type CodexContextWindowValue,
  type ModelInfo,
...
export const ModelConfigSelect = ({
  ...
  const codexContextLabel = codexContextWindowLoading
    ...
    : codexContextWindow === '1m'
      ? t('codexContextWindow.oneMillionLabel', { defaultValue: '1M' })
      : t('codexContextWindow.defaultLabel', { defaultValue: 'Default 272K' });
  ...
  }, [isOpen, mainRecalculate, showEffortRow, showSpeed, showPreset, showContextRow, showCodexContext]);
  ...
  return (
    <div style={WRAPPER_STYLE}>
      <ModelConfigTrigger
        buttonRef={buttonRef}
        isOpen={isOpen}
        summaryText={[summaryText, showCodexContext ? codexContextLabel : ''].filter(Boolean).join(' ')}
      ...
      {isOpen && (
        <ModelConfigDropdown
          ...
          showDivider={showDivider || showCodexContext}
          ...
          codexContextRow={(
            <ModelConfigSubmenuRow
              visible={showCodexContext}
              active={activeSubmenu === 'contextWindow'}
              testId="model-config-option-codex-context"
              label={t('codexContextWindow.title', { defaultValue: 'Context window' })}
              valueLabel={codexContextLabel}
              triggerRef={contextWindowTriggerRef}
              onHover={() => scheduleSubmenu('contextWindow')}
              onOpen={() => openSubmenu('contextWindow')}
            >
              {onCodexContextWindowChange && (
