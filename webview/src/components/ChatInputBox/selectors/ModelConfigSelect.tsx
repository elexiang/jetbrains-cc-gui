...
export const ModelConfigSelect = ({
  ...
  const triggerRefFor = (submenu: ActiveSubmenu) => {
    ...
    if (submenu === 'contextWindow') return contextWindowTriggerRef.current;
    return null;
  };
  ...
  const summaryParts = [
    ...
    showCodexContext ? codexContextLabel : '',
    ...
    <div style={WRAPPER_STYLE}>
      ...
      {isOpen && (
        ...
        >
          ...
          {showCodexContext && onCodexContextWindowChange && (
            <div
              ref={contextWindowTriggerRef}
              className={`selector-option${activeSubmenu === 'contextWindow' ? ' selected' : ''}`}
              data-testid="model-config-option-codex-context"
              onMouseEnter={() => scheduleSubmenu('contextWindow')}
              onClick={(event) => {
                event.stopPropagation();
                openSubmenu('contextWindow');
              }}
              style={OPTION_RELATIVE_STYLE}
            >
              <span style={OPTION_LABEL_STYLE}>{t('codexContextWindow.title', { defaultValue: 'Context window' })}</span>
              <div style={OPTION_VALUE_STYLE}>
                <span>{codexContextLabel}</span>
                <span className="codicon codicon-chevron-right" style={ARROW_ICON_STYLE} />
              </div>
              {activeSubmenu === 'contextWindow' && (
